...
template <class Num>
inline void Cmax(Num &x, const Num y) {
    x = y > x ? y : x;
}

template <class Num>
inline void Cmin(Num &x, const Num y) {
    x = y < x ? y : x;
}
...
int Merge(int x, int y) {
    ...
    if (tr[tr[p].ls].dis < tr[tr[p].rs].dis) {
...
int main()
{
    ...
    if (rt[1]) {
        q.push(make_pair(-tr[rt[1]].w, rt[1]));
    }
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (nxt) {
                q.push(make_pair(cur + tr[ed].w - tr[nxt].w, nxt));
            }
        }
    
        if (rt[tr[ed].ed]) {
            q.push(make_pair(cur - tr[rt[tr[ed].ed]].w, rt[tr[ed].ed]));
        }
    }
